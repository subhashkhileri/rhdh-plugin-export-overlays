...
| CI Diagnose | Auto-triggers via `ci-diagnose` label when curated PR checks go red | Diagnoses failing PR checks (Prow, GitHub Actions, comment-command statuses) and posts a read-only sticky diagnostic comment |
